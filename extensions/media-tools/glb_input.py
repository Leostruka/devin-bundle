...
def _accessor(gltf, bin_data, idx):
...
def load_glb(path, width=640, height=480, bg="#000000", wireframe=False):
    ...
    gltf, bin_data = _load(path)
    tris = []
    for mesh in gltf.get("meshes", []):
        for prim in mesh.get("primitives", []):
            if prim.get("mode", 4) != 4:
                continue
            pos = _accessor(gltf, bin_data, prim["attributes"]["POSITION"])
            idx = (_accessor(gltf, bin_data, prim["indices"]).astype(np.int64)
