...
Bundles skills, consolidated rules, config, hooks, scripts, playbooks, MCP, and credentials —
...
Done. Devin CLI now has 74 skills, 20 rules, 5 subagent profiles, 8 hook events, 15 hook scripts, 2 manual-run scripts, and playbooks configured.
...
flowchart TB
    subgraph Runtime["Devin CLI Runtime"]
        ...
        PLAYBOOKS[playbooks/<br/>reusable prompt templates]
        MCP[MCP + config]
    end
...
├── playbooks/           # reusable prompt templates (.devin.md)
├── mcp_config.json      # MCP server config (tokens MASKED by default)
├── credentials.toml     # API keys (generated by `export -NoMask`; ALL values MASKED by default)
...
|| PostToolUse | ^(exec\|mcp_call_tool)$ | context-pressure.py | Reports context pressure and high-cost tool use patterns (Rule 18) |
...
|| SessionEnd | all | memory-stop.py | Logs .devin/memory/ state and saves session artifacts |
...
7. **playbooks/** — install `.devin.md` prompt templates
...
6. **playbooks/** — copy `.devin.md` prompt templates
