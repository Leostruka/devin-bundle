# Mode: orchestration detail


# Universal bundle orchestrator

## TL;DR

1. Self-check (scope, skills, verify, no opinion).
2. Classify objective; route directly to matching skill(s).
3. Start with `skill-discovery`; use `ask-bundle` only if unclear.
...
Provide one entry point for the bundle's universal orchestration: classify the objective, invoke specialist skills directly, compose multi-skill flows, and retain control through verified completion. `ask-bundle` is the full routing reference, not a mandatory hop; `planning` and every other clear specialist route are first-class entry paths.

## When to use

- The user starts a session with "ask-bundle", a vague request, or no clear objective.
...
1. **Self-check before responding**
   ...
   - If the objective is unclear or the user just says "ask-bundle" / "start", ask the quick-start menu (see **Quick-start menu** below) with `ask_user_question` before routing.
...
2. **Direct routing and composition**
   ...
   - Start with `skill-discovery` to reinforce the skill-first rule.
   ...
   - If no bundled skill matches, use `skill-discovery` or `skill search`/`skill list`.
...
3. **Planning**
   ...
   - For explicit acceptance criteria, use `gates` or `gates`.
...
- Skills: discover via `skill-discovery` or `docs/SKILL-TIERS.md`.
...
Pick the entry skill from the user's situation. If the situation is not in this table, route to `ask-bundle` for the full map or `skill-discovery` for an external skill.

| Situation | Entry skill | Next |
|---|---|---|
| Build / change / implement something | `grilling` (With-docs) to capture intent and user impact, or `execution` if trivial | → `planning` Spec → Tickets (estimate PR size; split >500 lines) → `execution` + `testing` → `security` (if API, DB, secrets, endpoints, or infra) → `code-review` → `gates` → `finishing-a-development-branch` |
| Run AFK / unattended on a feature | `execution` only if issues exist and are `ready-for-agent`; otherwise use **Quick AFK issue creation** below | `execution` |
| Hard / intermittent / unclear bug | `debugging` | → `testing` regression → `architecture` if no seam |
| CI is failing | `debugging` | |
| Triage incoming issues / requests | `intake` | → `execution` |
| Large, foggy, multi-session effort | `planning` | → `planning` Spec → ... |
| Prototype to settle a design question | `prototype` (via `handoff`) | back to `grilling` / `planning` |
| Research / deep codebase exploration | `research` or `research` | feed into `grilling` or Spec |
| Need input from another person | `planning` Questionnaire | → `grilling` or Spec |
| Git merge / rebase conflict | `git-workflows` | |
| Improve architecture / find deep modules | `architecture` | → `grilling` if it generates an idea |
| Improve / evolve a skill, rule, hook, or MCP | `self-improvement` | → `writing-skills` if editing one skill, `devin-config` if adding a new capability |
| Release / deploy / rollback | `deploy` | → `gh` for GitHub operations, then smoke tests |
| Security assessment | `security` | → `execution` for approved remediations |
...
| API or database design | `api-spec` or `database` | → `execution` after contract/schema decisions |
| Performance or cost optimization | `performance` or `context-hygiene` | |
| Human-only procedure / provisioning | `wizard` | |
| Guided learning | `teach` | |
| Set up this repo for Devin | `project-bootstrap` or the engineering-skills setup | |
| Not sure which skill / flow fits | `ask-bundle` | full map |
| No skill matches | `skill-discovery` | evaluate / install |

For the main flow details (idea → ship, on-ramps, phase boundaries), see `ask-bundle`.

### Build/change flow gates

The default build/change flow is: `grilling` (intent and user impact) → `planning` (spec + tickets) → `execution` + `testing` → `security` (when the change touches API, DB, secrets, endpoints, or infrastructure) → `code-review` → `gates` → `finishing-a-development-branch`.
...
- **Size and boundaries**: `planning` estimates the PR size. If the estimate exceeds ~500 lines, split the task into smaller tickets with clear input/output boundaries. Target PR size is ~300 lines.
- **TDD**: `testing` is a default step, not optional. Tests verify intent; don't delete or skip them without explicit approval.
- **Pre-commit**: `project-bootstrap` runs lint-staged and tests before commit when the project is set up by `project-bootstrap` or `execution` touches pre-commit hooks.
- **Security audit**: `security` is a gate, not optional, whenever the change touches API, DB, secrets, endpoints, or infrastructure.
...
When the user says "ask-bundle", "start", or the objective is unclear, ask a focused `ask_user_question` with 2–4 options. The tool adds an "Other" option automatically; route "Other" to `ask-bundle` or a follow-up question.

- **Build or change something in the repo** → `grilling` (With-docs) if decisions remain, or `execution` if trivial
- **Improve / add a skill, rule, hook, or MCP** → `self-improvement`
- **Debug, fix, or research a problem** → `debugging` for bugs, `research` / `research` for exploration
- **Set up this repo for Devin or run AFK work** → `project-bootstrap` / the engineering-skills setup for setup, `execution` if issues already exist
...
2. If there is no spec, run `grilling` (With-docs mode) or `planning` (Spec mode) to write `.devin/scratch/<feature-slug>/spec.md`.
3. Run `planning` (Tickets mode) to break the spec into vertical-slice tracer bullets.
4. Publish one file per ticket to `.devin/scratch/<feature-slug>/issues/<NN>-<slug>.md`:
   ...
   - Use the local ticket template from `planning`.
...
6. Only then run `execution` if the user explicitly authorizes unattended work.
...
- Cross-skill: `ask-bundle` is the session-start wrapper. Use `skill-discovery` to reinforce skill-first behavior. Use `ask-bundle` for the full flow map when the quick router is not enough. Use `skill-discovery` for external or missing skills.
...
- Start `execution` without local issues in `ready-for-agent` state.
