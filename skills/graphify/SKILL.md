---
name: graphify
description: Use when the user asks about a codebase, architecture, file relationships, or project content, especially if graphify-out/ exists.
---
# /graphify
...
> **graphify needs no API key. Never ask the user for one, and never block on one.** Code is extracted structurally (AST) with no LLM and no key at all — a code-only corpus (the common `/graphify .` on a repo) skips semantic extraction entirely, so it needs nothing here: go straight to Part A and skip Part B. Semantic extraction (only for docs, papers, and images) runs through the host agent itself; dispatch `run_subagent` for that work. graphify does **not** read external provider keys. If you catch yourself about to prompt for, wait on, or stop because of a missing API key, that is a misread of this skill — proceed without one.

**Before semantic extraction:** no API check is required. If the corpus contains docs/papers/images, dispatch subagents to extract them inline or use any locally configured LLM integration the project already has. Do not ask the user for an API key.

> **No other API keys are read.** Semantic extraction falls to the host agent itself — the running session is the LLM. On a host that dispatches subagents, dispatch them as written in Part B. On a host that runs the CLI directly in a terminal and cannot dispatch subagents, do not stall: a code-only corpus has no semantic work, so write the empty semantic file (Part B "Fast path") and continue to Part C; for a corpus with docs/papers/images, extract those inline yourself, but in no case prompt for an external API key.
...
**MANDATORY: You MUST use the `run_subagent` tool here. Reading files yourself one-by-one is forbidden - it is 5-10x slower. If you do not use the `run_subagent` tool you are doing this wrong.**
...
Call the `run_subagent` tool multiple times IN THE SAME RESPONSE - one call per chunk. This is the only way they run in parallel. If you make one Agent call, wait, then make another, you are doing it sequentially and defeating the purpose.

**IMPORTANT - subagent type:** Always use `profile: "subagent_general"`. Do NOT use `Explore` - it is read-only and cannot write chunk files to disk, which silently drops extraction results. General-purpose has Write and Bash access which the subagent needs.

Concrete example for 3 chunks:
```
[`run_subagent` tool call 1: files 1-15, profile: "subagent_general"]
[`run_subagent` tool call 2: files 16-30, profile: "subagent_general"]
[`run_subagent` tool call 3: files 31-45, profile: "subagent_general"]
```
...
- If the file is missing, the subagent was likely dispatched as read-only (subagent_explore) — print a warning: "chunk N missing from disk — subagent may have been read-only. Re-run with subagent_general." Do not silently skip.
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort

If more than half the chunks failed or are missing, stop and tell the user to re-run and ensure `profile: "subagent_general"` is used.

Merge all chunk files into `.graphify_semantic_new.json`. **After each Agent call completes, read the real token counts from the `run_subagent` tool result's `usage` field and write them back into the chunk JSON before merging** — the chunk JSON itself always has placeholder zeros. Then run:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
from pathlib import Path
