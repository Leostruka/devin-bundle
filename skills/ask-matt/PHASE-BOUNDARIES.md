...
| **Clear context** | Empty the context window and start from nothing.                  |
| **`handoff`** | Write a portable markdown file and seed a session anywhere with it. |
| **Subagent** | Send a task to its own context window and get a report back.     |
| **Compact context** | Compress this context and seed a fresh session with the summary.  |
...
**2. Is the context irrelevant to what comes next?** Is everything in this session — the exploration, the decisions, the dead ends — disposable? If so, **clear context**. It is the cheapest move on the board: it takes no time and hands back the whole window. Clearing also isn't terminal — the old session stays resumable.
...
- swapping to a **new harness** (Devin → another tool),
...
**5. Otherwise, compact context.** Relevant context, same harness, same directory, and you need to stay in the loop — this is where the tree lands, and it lands here often. Pass it an instruction ("compact context — we're going to QA this area") so the summary keeps what the next phase needs.
