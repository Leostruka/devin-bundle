...
const Devin_VERSION = readDevinVersion();
const Devin_BRAND_IMAGE_URL = '';
const TELEMETRY_DISABLE_ENV_VARS = [
  'Devin_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'DEVIN_DISABLE_NONESSENTIAL_TRAFFIC'
];
const Devin_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function readDevinVersion() {
  ...
  for (const manifest of manifests) {
    try {
      ...
      // Packaged Devin plugins may omit package.json; try the next manifest.
    }
  }
...
function brandMarkup() {
  const version = escapeHtmlText(Devin_VERSION);
  const text = 'Devin v' + version;
  const logo = Devin_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + Devin_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(Devin_VERSION) + '" alt="Devin" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a href="#">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
