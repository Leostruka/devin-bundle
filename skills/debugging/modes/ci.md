...
- **`debugging`** — use for the root cause tracing methodology and disciplined diagnosis loop (red → minimise → hypothesise → instrument → fix → regression-test) when the CI failure is a real bug
- **`research`** — when the failure is intermittent, spans multiple jobs, or you need to correlate patterns across many historical builds
- **`testing`** — after fixing, write a regression test that would have caught the CI failure
- **`testing`** — after fixing, consider running mutation testing on the affected area to find more gaps
