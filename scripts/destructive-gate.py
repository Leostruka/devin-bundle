...
Gates (deterministic, read-only, fail-open):
  1. rm -rf with dangerous paths (/, ~, *, . alone, parent, system dirs)
...
# Explicitly dangerous rm targets
RM_DANGEROUS_TARGETS = {"/", "~", "$HOME", "*", ".", "..", "/*", "~/*", "./*"}
...
def check_rm_rf(command):
    """Return True when rm -rf targets a dangerous path."""
    m = RM_RF_RE.search(command)
    if not m:
        return False
    targets = split_targets(m.group(1))
    if not targets:
        return True  # rm -rf with no target is malformed and unsafe
